		...
		`}
			>
				<div className='flex flex-col h-full'>
					{/* Header */}
					<div className='p-4 pb-[27px] border-b border-cyan-200 flex justify-between items-center'>
						<h2 className='text-xl font-bold text-cyan-600'>Matches</h2>
						<button
					...
					<div className='flex-grow overflow-y-auto p-4 z-10 relative'>
						{isLoadingMyMatches ? (
							...
							matches.map((match) => (
								...
								<Link key={match[0]} to={`/chat/${match[0]}`}>
									<div className='flex items-center mb-4 cursor-pointer hover:bg-cyan-50 p-2 rounded-lg transition-colors duration-300'>
										<img
											src={match.image || "/avatar.png"}
											alt='User avatar'
											className='size-12 object-cover rounded-full mr-3 border-2 border-cyan-300'
										/>
			...
			<button
				className='lg:hidden fixed top-4 left-4 p-2 bg-cyan-500 text-white rounded-md z-0'
				onClick={toggleSidebar}
			>
...
const NoMatchesFound = () => (
	<div className='flex flex-col items-center justify-center h-full text-center'>
		<Heart className='text-cyan-400 mb-4' size={48} />
...
const LoadingState = () => (
	<div className='flex flex-col items-center justify-center h-full text-center'>
		<Loader className='text-cyan-500 mb-4 animate-spin' size={48} />
